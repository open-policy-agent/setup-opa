...
function mapArch(arch2) {
  return arch2 === "x64" ? "amd64" : arch2;
}
function mapOS(os2) {
  return os2 === "win32" ? "windows" : os2;
}
function maybeStatic(arch2, filename) {
  const staticBinary = core.getInput("static") === "true" || arch2 === "arm64";
  return staticBinary ? `${filename}_static` : filename;
}
function getDownloadObject(version2, mirror) {
  let vsn = `v${version2}`;
  let github2 = true;
  if (version2 === "latest" || version2 === "edge") {
  ...
  const arch2 = mapArch(os.arch());
  const filename = `opa_${mapOS(platform2)}_${arch2}`;
  const binaryName = platform2 === "win32" ? `${filename}.exe` : maybeStatic(arch2, filename);
  let url;
  if (github2) {
...
async function setup() {
  try {
    ...
    const download = getDownloadObject(version2, mirror);
    ...
    core.info(`Setup Open Policy Agent CLI version ${version2}`);
  } catch (e) {
    core.setFailed(e);
  }
}
